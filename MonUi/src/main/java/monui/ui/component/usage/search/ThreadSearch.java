...
public class ThreadSearch extends AbsSearch{
	YukDate date;
	
	public ThreadSearch(String caption, CastInfo info,CasteeInterface target) {
		...
		button.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				try {
					...
					helper.sendStack(info.session, info.name, date.getValue().getTime(),target);
				} catch (Exception e) {
