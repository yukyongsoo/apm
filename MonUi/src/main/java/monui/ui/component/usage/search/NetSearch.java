...
public class NetSearch extends AbsSearch{
	YukDate date;
	
	public NetSearch(String caption, CastInfo info,CasteeInterface target) {
		...
		button.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				try {
					...
					helper.sendNet(info.session, info.name, date.getValue().getTime(),target);
				} catch (Exception e) {
